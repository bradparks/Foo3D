...
class WebGLRenderDevice extends AbstractRenderDevice
{
    ...
    override function init():Void
    {
        ...
        */
        ...
        m_caps.maxVertAttribs = m_ctx.getParameter(m_ctx.MAX_VERTEX_ATTRIBS);
        m_caps.maxVertUniforms = m_ctx.getParameter(m_ctx.MAX_VERTEX_UNIFORM_VECTORS);
    ...
    override public function createTexture(_type:Int, _width:Int, _height:Int, _format:Int, _hasMips:Bool, _genMips:Bool, ?_hintIsRenderTarget=false):Int
    {
        ...
        switch (_format)
        {
            case RDITextureFormats.RGBA8, RDITextureFormats.RGBA16F: tex.glFmt = m_ctx.RGBA;
            case RDITextureFormats.DEPTH: tex.glFmt = m_ctx.DEPTH_COMPONENT;
        }        

        tex.glObj = m_ctx.createTexture();
        m_ctx.activeTexture(m_ctx.TEXTURE15);
    ...
    override public function uploadTextureData(_handle:Int, _slice:Int, _mipLevel:Int, _pixels:PixelData):Void
    {
        ...
        var inputFormat:Int = m_ctx.RGBA;
        var inputType:Int = m_ctx.UNSIGNED_BYTE;

        switch (tex.format)
        {
            case RDITextureFormats.RGBA16F:
                inputFormat = m_ctx.RGBA;
                inputType = m_ctx.FLOAT;
            case RDITextureFormats.DEPTH: 
                //inputFormat = m_ctx.DEPTH_COMPONENT;
                //inputType = m_ctx.FLOAT;
                throw "[Foo3D - ERROR] - TextureFormats.DEPTH not supported yet";
        }

        var target:Int = (tex.type == RDITextureTypes.TEX2D) ? 
            m_ctx.TEXTURE_2D : (m_ctx.TEXTURE_CUBE_MAP_POSITIVE_X + _slice);
    ...
    {
        // create shaders
        var vs:WebGLShader = m_ctx.createShader(m_ctx.VERTEX_SHADER);
        m_ctx.shaderSource(vs, _vertexShaderSrc);
        m_ctx.compileShader(vs);
        var success:Bool = m_ctx.getShaderParameter(vs, m_ctx.COMPILE_STATUS);
        ...
        var fs:WebGLShader = m_ctx.createShader(m_ctx.FRAGMENT_SHADER);
        m_ctx.shaderSource(fs, _fragmentShaderSrc);
        m_ctx.compileShader(fs);
        var success:Bool = m_ctx.getShaderParameter(fs, m_ctx.COMPILE_STATUS);
        if (!success && !m_ctx.isContextLost())
        {
        ...
        // create program
        var prog:WebGLProgram = m_ctx.createProgram();
        ...
        success = m_ctx.getProgramParameter(prog, m_ctx.LINK_STATUS);
        if (!success && !m_ctx.isContextLost())
        {
        ...
        var attribCount:Int = m_ctx.getProgramParameter(prog, m_ctx.ACTIVE_ATTRIBUTES);
        ...
        for (i in 0...m_numVertexLayouts)
        {
            ...
            {
                var info:WebGLActiveInfo = m_ctx.getActiveAttrib(prog, j);
    ...
    { 
        var shader:RDIShaderProgram = m_shaders.getRef(_handle);
        return m_ctx.getProgramParameter(shader.oglProgramObj, m_ctx.ACTIVE_UNIFORMS);
    }
    ...
    override public function getActiveUniformInfo(_handle:Int, _index:Int):RDIUniformInfo 
    {
        ...
        var info:WebGLActiveInfo = m_ctx.getActiveUniform(shader.oglProgramObj, _index);
        res.name = info.name;
        res.type = info.type;
        return res;        
    }
    ...
    { 
        ...
        if (_numColBufs > 0)
        {
            ...
            {
                m_ctx.bindFramebuffer(m_ctx.FRAMEBUFFER, rb.fbo);
                ...
                m_ctx.framebufferTexture2D(m_ctx.FRAMEBUFFER, m_ctx.COLOR_ATTACHMENT0 + j, m_ctx.TEXTURE_2D, tex.glObj, 0);
            }
        }
        // attach depth buffer
        if (_depth)
        {
            ...
            m_ctx.bindRenderbuffer(m_ctx.RENDERBUFFER, rb.depthBufObj);
            m_ctx.renderbufferStorage(m_ctx.RENDERBUFFER, m_ctx.DEPTH_COMPONENT16, rb.width, rb.height);
            // attach it
            m_ctx.framebufferRenderbuffer(m_ctx.FRAMEBUFFER, m_ctx.DEPTH_ATTACHMENT, m_ctx.RENDERBUFFER, rb.depthBufObj);
        }

        var rbObj:Int = m_renBuffers.add(rb);

        // validate fbo
        var status:Int = m_ctx.checkFramebufferStatus(m_ctx.FRAMEBUFFER);
        switch(status)
        {
            case m_ctx.FRAMEBUFFER_UNSUPPORTED: throw "Framebuffer is not supported";
            case m_ctx.FRAMEBUFFER_INCOMPLETE_ATTACHMENT: throw "Framebuffer incomplete attachment";
            case m_ctx.FRAMEBUFFER_INCOMPLETE_DIMENSIONS: throw "Framebuffer incomplete dimensions";
            case m_ctx.FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: throw "Framebuffer incomplete missing attachment";
        }
        if (status != m_ctx.FRAMEBUFFER_COMPLETE)
        {
    ...
    override public function bindRenderBuffer(_handle:Int):Void
    {
        ...
        if (_handle == 0)
        {
            ...
            m_ctx.bindFramebuffer(m_ctx.FRAMEBUFFER, null);
        }
        else
        {
    ...
    override public function getRenderBufferData(_handle:Int, ?_bufIndex:Int=0):RDIRenderBufferData
    {
        ...
        var format:Int = m_ctx.RGBA;
        var type:Int = m_ctx.UNSIGNED_BYTE;
    ...
    override function applyVertexLayout():Bool
    {
        ...
        for (i in 0...vl.numAttribs)
        {
            ...
            if (attribIndex >= 0)
            {
                ...
                m_ctx.bindBuffer(m_ctx.ARRAY_BUFFER, m_buffers.getRef(vbSlot.vbObj).glObj);
    ...
    override function applySamplerState(_tex:RDITexture):Void
    {
        ...
        var magFilters:Array<Int> = [m_ctx.LINEAR, m_ctx.LINEAR, m_ctx.NEAREST];
        var minFiltersMips:Array<Int> = [m_ctx.LINEAR_MIPMAP_NEAREST, m_ctx.LINEAR_MIPMAP_LINEAR, m_ctx.NEAREST_MIPMAP_NEAREST];
        var wrapModes:Array<Int> = [m_ctx.CLAMP_TO_EDGE, m_ctx.REPEAT, m_ctx.MIRRORED_REPEAT];

        if (_tex.hasMips)
            m_ctx.texParameteri(target, m_ctx.TEXTURE_MIN_FILTER, minFiltersMips[(state & AbstractRenderDevice.SS_FILTER_MASK) >> AbstractRenderDevice.SS_FILTER_START]);
        else
            m_ctx.texParameteri(target, m_ctx.TEXTURE_MIN_FILTER, magFilters[(state & AbstractRenderDevice.SS_FILTER_MASK) >> AbstractRenderDevice.SS_FILTER_START] );
        
        m_ctx.texParameteri( target, m_ctx.TEXTURE_MAG_FILTER, magFilters[(state & AbstractRenderDevice.SS_FILTER_MASK) >> AbstractRenderDevice.SS_FILTER_START] );
        m_ctx.texParameteri( target, m_ctx.TEXTURE_WRAP_S, wrapModes[(state & AbstractRenderDevice.SS_ADDRU_MASK) >> AbstractRenderDevice.SS_ADDRU_START] );
        m_ctx.texParameteri( target, m_ctx.TEXTURE_WRAP_T, wrapModes[(state & AbstractRenderDevice.SS_ADDRV_MASK) >> AbstractRenderDevice.SS_ADDRV_START] );
    }
    ...
    override public function commitStates(?_filter=0xFFFFFFFF):Bool
    {
        if ((m_pendingMask & _filter) != 0)
        {
            ...
            if ((mask & ARD.PM_CULLMODE) == ARD.PM_CULLMODE)
            {
                if (m_newCullMode != m_curCullMode)
                {
                    if (m_newCullMode == RDICullModes.NONE)
                        m_ctx.disable(m_ctx.CULL_FACE);
                    else
                    {
                        m_ctx.enable(m_ctx.CULL_FACE);
            ...
            if ((mask & ARD.PM_DEPTH_TEST) == ARD.PM_DEPTH_TEST)
            {
                if (m_newDepthTest != m_curDepthTest)
                {
                    if (m_newDepthTest == RDITestModes.DISABLE)
                    {
                        ...
                        {
                            m_ctx.disable(m_ctx.DEPTH_TEST);
            ...
            if ((mask & ARD.PM_INDEXBUF) == ARD.PM_INDEXBUF)
            {
                ...
                {
                    if (m_newIndexBuf != 0)
                        m_ctx.bindBuffer(m_ctx.ELEMENT_ARRAY_BUFFER, m_buffers.getRef(m_newIndexBuf).glObj);
                    else
                        m_ctx.bindBuffer(m_ctx.ELEMENT_ARRAY_BUFFER, null);
            ...
            if((mask & ARD.PM_BLEND) == ARD.PM_BLEND)
            {
                if (m_newSrcFactor != m_curSrcFactor || m_newDstFactor != m_curDstFactor)
                {
                    if (m_newSrcFactor == RDIBlendFactors.ONE && m_newDstFactor == RDIBlendFactors.ZERO)
                        m_ctx.disable(m_ctx.BLEND); // replace-function
                    else 
                    {
                        m_ctx.enable(m_ctx.BLEND);
            ...
            if((mask & ARD.PM_TEXTURES) == ARD.PM_TEXTURES)
            {
                for (i in 0...16)
                {
                    m_ctx.activeTexture(m_ctx.TEXTURE0+i);
    ...
    {
        if (commitStates())
            m_ctx.drawElements(_primType, _numInds, m_ctx.UNSIGNED_SHORT, _offset);
    }
